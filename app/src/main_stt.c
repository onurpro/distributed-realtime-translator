/**
 * @file main_stt.c
 * @brief Main entry point for Board 1 (Speech-to-Text).
 *
 * Captures audio and performs speech-to-text transcription using Whisper.
 * Sends transcribed segments to the Hub (Board 4).
 */
...
int main() {
  ...
  while (1) {
    ...
    while (1) {
      ...
      // Run Whisper with Streaming Callback
      if (STT_transcribe_file(buffer, result_buffer, MAX_TRANSCRIPTION_SIZE,
                              send_segment_callback) == 0) {
        // Send DONE signal
