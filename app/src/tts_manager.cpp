 ...
 * @brief Implementation of the TTS Manager using Piper.
 */
...
int TTS_speak(const char *text) {
  if (piper_exe.empty() || voice_model.empty())
    return -1;

  // Write text to a temporary file to avoid shell injection/quoting issues
  ...
  if (!f) {
    perror("[APP] Error: Could not create temp input file");
    return -1;
  }
  fprintf(f, "%s", text);
  fclose(f);
  ...
  std::string cmd = piper_exe + " --model " + voice_model +
                    " --length_scale 1.2 " + " --output_file " + output_wav +
                    " < " + temp_input;
