/**
 * @file translator_manager.cpp
 * @brief Implementation of the Translator Manager using Llama.cpp.
 */
...
/**
 * @brief Helper to add a token to a batch.
 */
void batch_add_token(llama_batch &batch, llama_token token, llama_pos pos,
                     bool logits) {
...
int Translator_process(const char *input, char *output, int max_len) {
  ...
  // Reset Brain (Critical for context clearing)
  if (ctx)
    llama_free(ctx);
  ...
  std::string prompt =
      ...
      "to French. Do not explain. Do not repeat the English.\n"
      "</s>\n"
      "<|user|>\n"
      "Hello world\n"
      "</s>\n"
  ...
  // Tokenize
  int n_prompt = -llama_tokenize(vocab, prompt.c_str(), prompt.length(), NULL,
                                 0, true, true);
  std::vector<llama_token> tokens_list(n_prompt);
  if (llama_tokenize(vocab, prompt.c_str(), prompt.length(), tokens_list.data(),
  ...
  for (int i = 0; i < max_len; i++) {
    llama_token new_token_id =
        llama_sampler_sample(smpl, ctx, batch.n_tokens - 1);
    ...
    if (n > 0) {
      ...
      if (piece.find('\n') != std::string::npos && result.length() > 5)
        break;
