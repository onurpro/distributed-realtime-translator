/**
 * @file tui_manager.c
 * @brief Implementation of the Text User Interface (TUI) Manager.
 */
...
/**
 * @brief Helper function to draw a box with a title.
...
/**
 * @brief Renders the log history in the Network Console panel.
 */
void render_logs() {
  int x = col_split + 1;
  int y_start = 4;
  ...
  while (drawn < log_count && drawn < LOG_DISPLAY_H) {
    mvprintw(y_start + LOG_DISPLAY_H - 1 - drawn, x, "%.*s", w,
             log_history[idx]);
...
void TUI_init(void) {
  ...
  // Header
  attron(COLOR_PAIR(4));
  for (int y = 0; y < HEIGHT_HEADER; y++)
    mvhline(y, 0, ' ', screen_w);
  mvprintw(1, screen_w / 2 - 15, "ENSC 351: DISTRIBUTED HUB");
  attroff(COLOR_PAIR(4));

  // Draw Static Boxes
  draw_box(3, 0, HEIGHT_PANEL, col_split, " BOARD 1: THE EAR ");
  draw_box(3 + HEIGHT_PANEL, 0, HEIGHT_PANEL, col_split,
           " BOARD 2: THE BRAIN ");
  draw_box(3 + HEIGHT_PANEL * 2, 0, HEIGHT_PANEL, col_split,
           " BOARD 3: THE MOUTH ");

  // Draw Log Box
  draw_box(3, col_split, HEIGHT_PANEL * 3, screen_w - col_split,
           " NETWORK CONSOLE ");
